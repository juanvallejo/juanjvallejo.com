(function($){
	...
	var defaults = {
		...
		resultComments :  
	  {
	  	...
	  	formSettings: {
	  		showForm:true,
	  		formTitle:'Fill out this quick form!',
	  		formText:'Fill this out!',
	  	},
	  	formInputs:null
    ...
    for (questionsIteratorIndex = 0; questionsIteratorIndex < config.questions.length; questionsIteratorIndex++) {
        ...
        if (questionsIteratorIndex < config.questions.length - 1) {
            contentFob += '<div class="next"><a class="nav-next" href="#">Next</a></div>';
 			...
 			if(config.formSettings.showForm) {
 				contentFob += '<div class="next"><a class="nav-next" href="#">Next</a></div>';
 			} else {
    ...
    if(config.formSettings.showForm) {
    	var value;
		var formFob = '<div class="slide-container"><div class="question-number">'+config.formSettings.formText+'</div><div class="question">'+config.formSettings.formTitle+'</div><ul class="answers" style="width:100%;display:block;">';
       	formFob += '<form action="#" class="form">';
       	for(var i=0;i<config.formInputs.inputNumber;i++) {
       		var inputClass = 'inputItem'+i;
       		var value = '';
       		var display = 'block';
       		var lineBreak = '';
       		if(config.formInputs.inputTypes[i] == "checkbox" || config.formInputs.inputTypes[i] == "radio") {
       			value = config.formInputs.inputValues[i];
       			formFob += '<label for="'+inputClass+'">'+config.formInputs.inputValues[i]+'</label>';
       			display = 'inline;';
       			lineBreak = '<br />';
       		}
       		if(config.formInputs.inputTypes[i] == "textarea") {
       			var rows = config.formInputs.inputPlaceholders[i].split(",")[0].split(":")[1];
       			var cols = config.formInputs.inputPlaceholders[i].split(",")[1].split(":")[1];
       			formFob += '<textarea cols="'+cols+'" rows="'+rows+'" name="'+config.formInputs.inputPlaceholders[i]+'" placeholder="'+config.formInputs.inputValues[i]+'"></textarea>';
       		} else {
       			formFob += '<input style="display:'+display+'" type="'+config.formInputs.inputTypes[i]+'" placeholder="'+config.formInputs.inputValues[i]+'" class="'+inputClass+'" name="'+config.formInputs.inputNames[i]+'" value="'+value+'"/>'+lineBreak;
       		}
       	}
       	formFob += '</form>';
        formFob += '</ul><div class="nav-container"><div class="prev"><a class="nav-previous" href="#">Prev</a></div>';
        formFob += '<div class="next final"><a class="nav-show-result" href="#">Finish</a></div></div></div></div>';
    } else {
    	formFob = '';
    }
    if(config.debugMode) {
    ...
    superContainer.html(introFob + contentFob + formFob + exitFob);
    ...
    superContainer.find('.next').click(function() {
		if(config.formSettings.showForm === false) {
    ...
    superContainer.find('.final').click(function() {
        ...
        score = roundReloaded(trueCount / questionLength * 100, 2);
		  ...
		  {
		  shareButton += '<a href="http://www.facebook.com/sharer/sharer.php?s=100&p[title]='+config.socialStatus.replace("{score}", score)+'&p[url]= '+config.shortURL+' &p[summary]=yoursummary&p[images][0]=yourimage" class="share-button-fb" target="_blank" ><img src="'+config.facebookImage+'" /></a><div class="jquizzy-clear"></div>';
		  }
